...
## 1. Doplnit údaje na webu
V `index.html` nahraďte zástupné texty (hledejte je přes Ctrl+F):

| Hledat | Nahradit za |
|---|---|
| `VAŠE MĚSTO` | vaše město (je v titulku, popisu, ceníku, kontaktu i v datech pro Google) |
...
## 2. Doména a hosting
- **Doména:** podle registru CZ.NIC byla `dilnamiru.cz` v době tvorby webu volná. Registrovat ji můžete u kteréhokoli registrátora (Wedos, Forpsi, Active24 …) za zhruba 200–300 Kč ročně.
- **Hosting zdarma:** celý web je statický (soubory `index.html`, `style.css`, `favicon.svg`, `robots.txt`, `sitemap.xml`), takže stačí:
  - **Cloudflare Pages** nebo **Netlify:** složku přetáhnete myší v prohlížeči (funkce „Deploy / Upload assets“) a v nastavení připojíte vlastní doménu,
  - případně webhosting u registrátora domény: soubory nahrajete přes FTP.

## 3. Google – Profil firmy (nejdůležitější krok)
Díky tomuto profilu se objevíte v Mapách a na dotaz „hodinový manžel + město“.
...
5. **Oblast působnosti:** vaše město a okolní obce.
...
Na https://search.google.com/search-console přidejte doménu a odešlete `sitemap.xml`. Web se tak dostane do vyhledávání rychleji.

## 5. Další místa, kde se vyplatí být
- **Firmy.cz (Seznam):** v Česku stále hodně používané, registrace je zdarma.
- Facebook stránka a místní skupiny („Hodinový manžel [město]“, „Bazar [město]“).
